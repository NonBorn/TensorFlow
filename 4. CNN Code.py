...
train_path = '/Users/nonborn/Desktop/Dataset/Numpy/Train'
train_path_labels = '/Users/nonborn/Desktop/Dataset/Numpy/Train_Labels'
test_path = '/Users/nonborn/Desktop/Dataset/Numpy/Test'
test_path_labels = '/Users/nonborn/Desktop/Dataset/Numpy/Test_Labels'
...
def seq_batch(fpath_train, fpath_labels, b_size, ind):
    index = range(ind, ind+b_size)  # get a number of images per class - indexing
...
def seq_batch2(fpath_train, fpath_labels, b_size, ind, images):
    x = 1
    img_path = []
    k = 0
    pointer = []
    for r in range(0, num_of_classes):
        pointer = pointer + range(k + ind, k + ind + images)
        k += train_block
    #print(pointer)
    img_path = [fpath_train + '/' + os.listdir(train_path)[x] for x in pointer]
    print(img_path)
    return x
...
for i in range(119, 122):
    x = seq_batch2(train_path, train_path_labels, batch_size, g_index, img_per_class)
    cnt+=1
    g_index += img_per_class
    if g_index % 480 == 0:
        g_index = 0
        epoch += 1
print(cnt)
...
#
# path_init = '/Users/nonborn/Desktop/tx_000100000000/Initial Train'
# tt = exclude_os_files(path_init)
#
#
# def test_class(tpath, tlpath, cl):
#     # Regex expression for hidden mac os files
#     regexp = re.compile("{}".format(cl))
#     test_features = [x for x in tpath if bool(re.search(regexp, x))]
#     test_labels = [y for y in tlpath if bool(re.search(regexp, y))]
#     test_features = [np.load(test_features[x]) for x in range(0, len(test_features))]
#     test_labels = [np.load(test_labels[x]) for x in range(0, len(test_labels))]
...
#         batch_x, batch_y = seq_batch(train_path, train_path_labels, batch_size, g_index)
#         g_index = g_index + batch_size + 1
...
#             )
#             if step % 10 == 0:
#                 xt, yt = test_class(test_img_path, test_label_path, 'airplane')
#                 t_acc = sess.run(accuracy, feed_dict={x: xt, y: yt, keep_prob: 1.})
#                 print(
